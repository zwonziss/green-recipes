...
- **Windows support is newer and less battle-tested than Linux, on both CPU and CUDA.** Confirmed on a Windows CPU-only machine while writing this recipe: TorchInductor's CPU backend shells out to a C/C++ compiler (`cl.exe` on Windows) to build the generated kernel, and without Visual Studio's Build Tools installed and on `PATH`, compilation raises `InductorError: Compiler: cl is not found` on the very first compiled step. This script catches that (and any other compile-time failure) and reports only the eager receipt rather than crashing — treat a skip here as "your toolchain is missing a compiler," not as a correctness problem. CUDA Triton on Windows has similarly newer, rougher support than Linux.
